...
Bitcoin.prototype._parseInput = function(txIsCoinbase, vin, index) {
  ...
  if (vin.scriptSig && vin.scriptSig.hex) {
    const script = bitcore.Script.fromHex(vin.scriptSig.hex);
...
Bitcoin.prototype._scanBlockTx = function(block, txid) {
  ...
  const storeSentFromInputs = (inputs) => {
    return Promise.all(inputs.map((input, index) => {
      ...
      return Promise.all([
        (input.satoshis !== null ? this.db.incrbyAsync(`address:${input.address}:sent`, input.satoshis) : null),
  ...
  Promise.map(addressArg, address =>
    ...
    .then(results => {
      ...
      callback(null, {
        ...
        items: this._pageArray(200, options, txs),
      });
    })
    .catch(callback);
};

Bitcoin.prototype._pageArray = (max, options = {}, array) => {
  const from = options.from === undefined ? 0 : options.from;
  const to = options.to === undefined ? Math.min(array.length, from + max) : options.to;
...
Bitcoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  ...
  return Promise.all([
    this.db.smembersAsync(`address:${address}:txids`),
    this.db.getAsync(`address:${address}:sent`),
    this.db.getAsync(`address:${address}:received`),
  ])
    .then(([txids, sent, received]) => {
      const balance = (received || 0) - (sent || 0);
      ...
      return callback(null, {
        ...
        txids: this._pageArray(200, options, txids),
      });
    })
    .catch(callback);
