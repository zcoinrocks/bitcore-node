...
Bitcoin.prototype.start = function(callback) {
  ...
  if (process.env.HEROKU_REDIS_ORANGE_URL) {
    this.db = redis.createClient({ url: process.env.HEROKU_REDIS_ORANGE_URL });
  } else {
...
Bitcoin.prototype.stop = function(callback) {
  this.stopping = true;
  log.info('[bitcoin] stop doing nothing on purpose');
};
